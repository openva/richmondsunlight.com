...
const SEARCHD_COMMAND_PING = 9;

/// current client-side command implementation versions
const VER_COMMAND_SEARCH = 0x120;
const VER_COMMAND_EXCERPT = 0x105;
const VER_COMMAND_UPDATE = 0x104;
...
const SPH_RANK_PROXIMITY_BM15 = 0;    ///< default mode, phrase proximity major factor and BM15 minor one
const SPH_RANK_BM15 = 1;    ///< statistical mode, BM15 ranking only (faster but worse quality)
const SPH_RANK_NONE = 2;    ///< no ranking, all matches get a weight of 1
const SPH_RANK_WORDCOUNT = 3;    ///< simple word-count weighting, rank is a weighted sum of per-field keyword occurrence counts
...
const SPH_RANK_PROXIMITY_BM25 = 0;    ///< alias for PROXIMITY_BM15; to be retired
const SPH_RANK_BM25 = 1;    ///< alias for BM15; to be retired
...
class SphinxClient
{
	...
	{
		$known_names = array ( "reverse_scan", "sort_method", "max_predicted_time", "boolean_simplify", "idf", "global_idf", "low_priority", "lax_agent_errors" );
		$flags = array (
		...
		"lax_agent_errors" => array ( true, false )
		);
		...
		if ( $flag_name=="lax_agent_errors" ) $this->_query_flags = sphSetBit ( $this->_query_flags, 10, $flag_value );
	}
	...
	function AddQuery ( $query, $index="*", $comment="" )
	{
		...
		if ( $this->_predictedtime>0 )
			$req .= pack ( "N", $this->_predictedtime);
	...
	function Ping ()
	{
		...
		$req = pack ( "nnNN", SEARCHD_COMMAND_PING, VER_COMMAND_PING, 4, 0 );
		if ( !( $this->_Send ( $fp, $req, 12 ) ) ||
			 !( $response = $this->_GetResponse ( $fp, VER_COMMAND_PING ) ) )
			return false;
		...
		list(, $ha_weight) = unpack ( "N", $response );
		return (int) $ha_weight >= 0;
	}
